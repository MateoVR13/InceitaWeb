...
// Reusable Components with new Emerald Palette and Apple-like aesthetic
...
const GlassButton: React.FC<{onClick?: (e: MouseEvent<HTMLButtonElement>) => void, children: ReactNode, className?: string, type?: "button" | "submit" | "reset"}> = ({ onClick, children, className = '', type = 'button' }) => (
    <button
      type={type}
      onClick={onClick}
      className={`glass-button bg-emerald-500 hover:bg-emerald-700 border-transparent text-white font-semibold px-8 py-3 rounded-lg shadow-lg transition-all duration-300 ${className}`}
    >
      {children}
    </button>
);
                    ...
                    <div className="hidden md:flex items-center space-x-1">
                        {navLinks.map((link) => (
                            <a
                                ...
                                className={`px-4 py-2 rounded-md text-sm font-medium transition-all duration-300 ${currentPage === link.page ? 'bg-emerald-500/10 text-emerald-700 font-semibold' : 'text-gray-500 hover:bg-gray-400/10 hover:text-apple-black'}`}
                            >
                                {link.name}
                            </a>
                        ))}
                            ...
                            {navLinks.map((link) => (
                                <li key={link.name}>
                                    <a
                                        href="#"
                                        onClick={(e) => handleNavClick(e, link.page)}
                                        className={`block px-4 py-3 rounded-lg text-lg font-medium transition-colors ${currentPage === link.page ? 'bg-emerald-500/20 text-emerald-700' : 'text-apple-black hover:bg-gray-400/10'}`}
                                    >
                                        {link.name}
                                    </a>
                                </li>
...
const StatCard: React.FC<{ stat: Stat, delay: number }> = ({ stat, delay }) => (
  <div style={{ animationDelay: `${delay}ms` }} className="glass-card p-6 text-center transition-transform duration-300 hover:-translate-y-2 hover:shadow-neon-emerald opacity-0 animate-fade-in-up">
    <p className="text-5xl font-bold text-emerald-500">{stat.value}</p>
...
const ProjectCard: React.FC<{ project: Project; onClick: () => void }> = ({ project, onClick }) => (
  <div onClick={onClick} className="glass-card cursor-pointer overflow-hidden group flex flex-col h-full transition-all duration-500 hover:shadow-neon-emerald hover:-translate-y-2 hover:rotate-[-1deg]">
    ...
    <div className="p-6 flex-grow flex flex-col">
        ...
        <span className="text-emerald-500 font-semibold self-start group-hover:underline">Ver más &rarr;</span>
    </div>
  </div>
);
    ...
    <div className="fixed inset-0 bg-black/40 z-[100] flex items-center justify-center p-4 animate-fade-in-up" onClick={onClose}>
      <div className="glass-card w-full max-w-4xl max-h-[90vh] overflow-y-auto m-auto" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="p-8 md:p-12">
          ...
          <div className="grid md:grid-cols-2 gap-8">
            <div>
                ...
                <div className="flex flex-wrap gap-2">
                    {project.technologies.map(tech => <span key={tech} className="bg-emerald-500/20 text-emerald-700 text-sm font-medium px-3 py-1 rounded-full">{tech}</span>)}
                </div>
            </div>
            <div>
          ...
          {project.projectUrl && (
            <div className="mt-8 border-t border-gray-200/60 pt-6 flex justify-start">
                <a
                    href={project.projectUrl}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="glass-button bg-emerald-500 hover:bg-emerald-700 border-transparent text-white font-semibold px-6 py-3 rounded-lg shadow-lg transition-all duration-300 inline-flex items-center gap-2"
                >
                    Visitar Proyecto
...
const TeamMemberCard: React.FC<{ member: TeamMember, delay: number }> = ({ member, delay }) => (
  <div style={{ animationDelay: `${delay}ms` }} className="glass-card text-center transition-all duration-300 hover:scale-105 hover:shadow-neon-emerald pt-16 p-6 flex flex-col h-full relative animate-fade-in-up opacity-0">
    ...
    <div className="flex-grow flex flex-col justify-center">
        ...
        <p className="text-emerald-500 font-semibold text-sm mt-1">{member.title}</p>
    ...
    >
        <div className="glass-card p-8 md:p-12 grid md:grid-cols-2 gap-12 items-center">
            <div className="relative animate-fade-in-up">
                <div className="absolute -inset-2 bg-gradient-to-r from-emerald-400 to-green-400 rounded-2xl transform -rotate-3 opacity-30 blur-lg"></div>
            ...
            <div className="animate-fade-in-up" style={{animationDelay: `150ms`}}>
                ...
                <ul className="mt-6 space-y-4 text-gray-700">
                    <li className="flex items-start gap-3"><div className="w-2 h-2 mt-2 rounded-full bg-emerald-500 shrink-0"></div><span>Ciencia de datos aplicada a la optimización de procesos agrícolas y sostenibles.</span></li>
                    <li className="flex items-start gap-3"><div className="w-2 h-2 mt-2 rounded-full bg-emerald-500 shrink-0"></div><span>Desarrollo y calibración de redes de sensores IoT para monitoreo de alta precisión.</span></li>
                    <li className="flex items-start gap-3"><div className="w-2 h-2 mt-2 rounded-full bg-emerald-500 shrink-0"></div><span>Modelado con Inteligencia Artificial para la predicción y prevención en entornos complejos.</span></li>
                    <li className="flex items-start gap-3"><div className="w-2 h-2 mt-2 rounded-full bg-emerald-500 shrink-0"></div><span>Análisis termodinámico y de disolución para la industria farmacéutica y de alimentos.</span></li>
                </ul>
            </div>
        </div>
    </PageWrapper>
  ...
  >
    <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
      {TRAINING_COURSES.map((course, index) => (
        <div key={course.title} className="glass-card p-8 text-center flex flex-col items-center transition-all duration-300 hover:-translate-y-2 hover:shadow-neon-emerald animate-fade-in-up opacity-0" style={{animationDelay: `${index * 100}ms`}}>
          {course.icon}
          ...
          <div className="mt-4 flex flex-wrap gap-2 justify-center">
            {course.tags.map(tag => (
              <span key={tag} className="bg-emerald-500/20 text-emerald-700 text-xs font-medium px-2 py-1 rounded-full">{tag}</span>
            ))}
          </div>
        </div>
      ))}
        ...
        <div className="glass-card max-w-4xl mx-auto p-8 md:p-12 grid lg:grid-cols-2 gap-12 items-center">
            <div className="animate-fade-in-up">
                ...
                <div className="mt-6 space-y-4 text-gray-700">
                    <p className="flex items-center gap-3"><svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-emerald-500" viewBox="0 0 20 20" fill="currentColor"><path d="M2.003 5.884L10 9.882l7.997-3.998A2 2 0 0016 4H4a2 2 0 00-1.997 1.884z" /><path d="M18 8.118l-8 4-8-4V14a2 2 0 002 2h12a2 2 0 002-2V8.118z" /></svg><span>contacto@inceita.com</span></p>
                    <p className="flex items-center gap-3"><svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-emerald-500" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M5.05 4.05a7 7 0 119.9 9.9L10 18.9l-4.95-4.95a7 7 0 010-9.9zM10 11a2 2 0 100-4 2 2 0 000 4z" clipRule="evenodd" /></svg><span>Bogotá, Colombia</span></p>
                </div>
                <div className="mt-8">
               ...
               <form>
                    <div className="space-y-6">
                        <div>
                            ...
                            <input type="text" id="name" placeholder="Su nombre" className="glass-input mt-1 block w-full px-4 py-3 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/50 text-apple-black" />
                        </div>
                        <div>
                            <label htmlFor="email" className="block text-sm font-semibold text-gray-700">Correo electrónico</label>
                            <input type="email" id="email" placeholder="su@email.com" className="glass-input mt-1 block w-full px-4 py-3 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/50 text-apple-black" />
                        </div>
                        <div>
                            <label htmlFor="message" className="block text-sm font-semibold text-gray-700">Mensaje</label>
                            <textarea id="message" rows={4} placeholder="¿En qué podemos ayudarle?" className="glass-input mt-1 block w-full px-4 py-3 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/50 text-apple-black"></textarea>
                        </div>
